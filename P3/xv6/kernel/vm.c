...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  if(newsz > USERTOP- PGSIZE* counter)
    return 0;
  if(newsz < oldsz)
    return oldsz;
...
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  for(; a  < oldsz; a += PGSIZE){
    ...
    if(pte && (*pte & PTE_P) != 0){
      ...
      if(pa == 0)
        panic("kfree");
      if (pa != (unsigned int)shmem_addr[0] && pa != (unsigned int)shmem_addr[1] && pa != (unsigned int)shmem_addr[2] && pa != (unsigned int)shmem_addr[3]){
        kfree((char*)pa);
        *pte = 0;
      }
    }
...
int shmem_access_helper(int page_number){
  int i=0;
  int maximum = 0;
  if (proc->pages[page_number] !=0){
    return USERTOP - PGSIZE * proc->pages[page_number];
  }
  for (i=0; i<SHMEM_PAGES; i++){
    if (proc->pages[i]>maximum){
      maximum = proc->pages[i];
    }
  }
  if (proc->sz > USERTOP-PGSIZE*(maximum+1)){return 0;}
  proc->pages[page_number] = maximum +1;
  if (mappages(proc->pgdir,(void*)(USERTOP - PGSIZE*proc->pages[page_number]),PGSIZE,PADDR(shmem_addr[page_number]),PTE_W|PTE_U) < 0){
    return 0;
  }
...
int shmem_count_helper(int page_number){
