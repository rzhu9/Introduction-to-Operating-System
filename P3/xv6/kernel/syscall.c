...
fetchint(struct proc *p, uint addr, int *ip)
{
  ...
  if(addr >= p->sz || addr+4 > p->sz){
    if (addr < USERTOP - PGSIZE*(counter) || addr > USERTOP){
      return -1;
    }
  }
  *ip = *(int*)(addr);
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if(addr >= p->sz || addr+4 > p->sz){
    if (addr < USERTOP - PGSIZE*(counter) || addr > USERTOP || addr +4 < USERTOP - PGSIZE*(counter) || addr +4 > USERTOP){
      return -1;
    }
  }
  *pp = (char*)addr;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if((uint)i >= proc->sz || (uint)i+size > proc->sz || i<4096){
    if ((uint)i < USERTOP - PGSIZE*(counter) || (uint)i > USERTOP || (uint)i +4 < USERTOP - PGSIZE*(counter) || (uint)i +4 > USERTOP){
      return -1;
    }
  }
  *pp = (char*)i;
